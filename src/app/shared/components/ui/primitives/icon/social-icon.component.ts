...
function getPlatform(name: string): SocialPlatform {
...
@Component({
  ...
  template: `
    <svg [attr.width]="size()" [attr.height]="size()" viewBox="0 0 24 24" fill="currentColor" [attr.class]="className()">
...
export class SocialIconComponent {
  ...
  readonly platform = input<SocialPlatform>();
  readonly size = input<number>(20);
  readonly className = input<string>('');

  readonly iconPath = computed(() => {
    const explicitPlatform = this.platform();
    const detectedPlatform = explicitPlatform || getPlatform(this.name());
    return SOCIAL_ICONS[detectedPlatform];
  });
}
